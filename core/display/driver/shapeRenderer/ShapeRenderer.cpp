...
void pico_ssd1306::drawCircle(pico_ssd1306::SSD1306 *ssd1306, uint8_t x_center, uint8_t y_center, uint8_t radius, pico_ssd1306::WriteMode mode)
{
  int x = radius;
  int y = 0;
  int err = 0;
  ...
  while (x >= y)
  {
    ssd1306->setPixel(x_center + x, y_center + y, mode);
    ssd1306->setPixel(x_center + y, y_center + x, mode);
    ssd1306->setPixel(x_center - y, y_center + x, mode);
    ssd1306->setPixel(x_center - x, y_center + y, mode);
    ssd1306->setPixel(x_center - x, y_center - y, mode);
    ssd1306->setPixel(x_center - y, y_center - x, mode);
...
void pico_ssd1306::fillCircle(pico_ssd1306::SSD1306 *ssd1306, uint8_t x_center, uint8_t y_center, uint8_t radius, pico_ssd1306::WriteMode mode)
{
  int x = radius;
  int y = 0;
  int err = 0;
  ...
  while (x >= y)
  {
    ...
    {
      ssd1306->setPixel(i, y_center + y, mode);
      ssd1306->setPixel(i, y_center - y, mode);
    }
    for (int i = x_center - y; i <= x_center + y; i++)
    {
      ssd1306->setPixel(i, y_center + x, mode);
      ssd1306->setPixel(i, y_center - x, mode);
    }
